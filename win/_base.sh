...
function build_pacman {
    pacman  --disable-download-time --root "${BUILD_ROOT}" "$@"
}

function build_pip {
      export http_proxy=http://10.52.150.150:8080
      export https_proxy=https://10.52.150.150:8080      
      "${BUILD_ROOT}"/"${MINGW}"/bin/"${PYTHON_ID}".exe -m pip  --proxy https://10.52.150.150:8080 "$@"
}
...
function install_pre_deps {
    pacman -S --needed --noconfirm --disable-download-time p7zip \
...
function create_root {
    ...
    #build_pacman -Syu --disable-download-time
    #build_pacman --noconfirm --disable-download-time  -S base
}
